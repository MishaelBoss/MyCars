...
public class ControllCamera : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        RaycastHit hit;
        if (Physics.Raycast(target.position, transform.position - target.position, out hit, Vector3.Distance(transform.position, target.position), masktack))
        {
